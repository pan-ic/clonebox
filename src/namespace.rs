...
pub(crate) fn pre_pivot_mount(bundle_path: &str, new_root: &str, mounts: &Vec<Mount>) -> String {
    mount_child(
        Some(new_root),
        new_root,
        None::<&str>,
    ...
    for mount in mounts {
        if mount
            .options
            .as_ref()
            .map(|o| o.contains(&"bind".to_string()))
            .unwrap_or(false)
        {
            let options_formatted_string: Option<String> =
                mount.options.as_ref().map(|o| o.join(","));
    ...
    child_try!(create_dir_all(Path::new(&merged)), "create_dir_all", 1);

    let options_formatted_string = format!(
        "lowerdir={},upperdir={},workdir={}",
        new_root, upper_dir, work_dir
    );
...
pub(crate) fn do_pivot_root(new_root: &str) {
    ...
    child_try!(
        pivot_root(new_root, child_old_path.as_str()),
        "pivot_root",
        1
    );
...
pub(crate) fn mount_child(
    ...
    child_try!(mount(src, dest, mount_type, flags, data,), msg, 1);
}

pub(crate) fn post_pivot_mount(mounts: &Vec<Mount>) {
    for mount in mounts {
        let (mount_flag, msg) = match mount.mount_type.as_deref() {
            Some("tmpfs") => (MsFlags::empty(), "tmpfs"),
            _ => (MsFlags::empty(), "skip"),
        };
