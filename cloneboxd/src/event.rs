...
use tokio::{
    io::{BufReader, AsyncBufRead, AsyncBufReadExt},
    net::{
        UnixListener,
        UnixSocket,
        UnixStream,
...
use crate::daemon::ContainerEntry;
...
fn update_status(event: Event,
    containers: &Arc<Mutex<HashMap<String, ContainerEntry>>>) -> anyhow::Result<()> {    
    ...
    if let Some(entry) = h.get_mut(&event.get_id()) {
        ...
        if entry.state != ContainerState::Creating {
            if let Some(tx) = entry.created_waiter.take() { 
                let _ = tx.send(()); 
            }
        }
...
pub(crate) async fn event_loop(listener: UnixListener, containers: &Arc<Mutex<HashMap<String, ContainerEntry>>>) -> anyhow::Result<()> {
...
pub(crate) fn get_app_data_path() -> String {
